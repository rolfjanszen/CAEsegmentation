...
class CAE:
    ...
    opim_func = None
    cost = None
    def __init__(self, inp_sz_, outp_sz_, save_path =''):
        self.inp_sz =inp_sz_
    ...
    def autoEncoder(self, inp_):
        ...
        decode_3 = tf.layers.conv2d_transpose(inputs=conc_enc_3, filters=1, kernel_size=9, strides=(1,1),padding='valid')
        decode_3 = tf.nn.tanh(decode_3)
        print('decode_3 ', decode_3.shape)
        output = tf.reshape(decode_3,shape=[-1,self.outp_sz,self.outp_sz])
        return  output
    ...
    def test(self, input, expected_out):
        ...
        out = self.sess.run(tf.nn.sigmoid(self.model),feed_dict={self.im_in:input})

        gray_scale_out = np.array(out*255, dtype=np.uint8)
        gray_scale_expect = np.array(expected_out*255, dtype=np.uint8)

        im_result = np.concatenate((input, gray_scale_expect, gray_scale_out))
